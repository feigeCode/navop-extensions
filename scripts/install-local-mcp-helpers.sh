...
usage() {
  cat >&2 <<'EOF'
Usage: scripts/install-local-mcp-helpers.sh [helper-id]

Build, package, verify, and install MCP helper extensions into the local one-hub
MCP helper directory. Passing a helper id installs only that helper; omitting it
...
helper_ids() {
  ...
  find "${REPO_DIR}/extensions/mcp-helper" -mindepth 2 -maxdepth 2 -name extension.build.json -print \
    | sort \
...
package_helper() {
  local id="$1"
  local target="$2"
  local manifest_json="${REPO_DIR}/extensions/mcp-helper/${id}/mcp_helper.json"
  local version

  [ -f "$manifest_json" ] || fail "missing MCP helper manifest: ${manifest_json}"
  version="$(json_value "$manifest_json" 'data.version')"
  [ -n "$version" ] || fail "missing version in ${manifest_json}"

  mkdir -p "$ARTIFACT_DIR"
  printf 'Packaging %s %s\n' "$id" "$version"
  bash "${SCRIPT_DIR}/package-mcp-helper.sh" "$id" "$target" "$ARTIFACT_DIR" "$version"
}

install_packaged_helper() {
  local id="$1"
  local target="$2"
  ...
  [ -d "$packaged_dir" ] || fail "packaged MCP helper directory does not exist: ${packaged_dir}"
  ...
  if ! cp -R "${packaged_dir}/." "${dest_dir}/"; then
    ...
    fail "failed to install ${id}; restored previous helper if a backup existed"
  fi
...
main() {
  ...
  printf 'Installing local MCP helpers into %s\n' "$INSTALL_ROOT"

  while IFS= read -r id; do
    [ -n "$id" ] || continue
    metadata="${REPO_DIR}/extensions/mcp-helper/${id}/extension.build.json"
    [ -f "$metadata" ] || fail "missing extension build metadata: ${metadata}"
    target="$(target_for_helper "$metadata")"
    build_helper "$id" "$target" "$metadata"
    archive="$(package_helper "$id" "$target" | tail -n 1)"
    bash "${SCRIPT_DIR}/verify-mcp-helper-package.sh" "$archive"
    install_packaged_helper "$id" "$target"
  done < <(helper_ids "$selected")
}
