//! invoke 大结果的 blob 存储(仿 elasticsearch provider 的 state/blob.rs)。
...
#[derive(Debug)]
pub(crate) struct ProviderBlob {
    resource_id: String,
    data: Vec<u8>,
    offset: usize,
}

/// blob 表(资源关闭时按 resource_id 批量释放)
#[derive(Debug, Default)]
pub(crate) struct ProviderBlobTable {
    blobs: HashMap<String, ProviderBlob>,
}

impl ProviderBlobTable {
    ...
    fn store(&mut self, resource_id: &str, data: Vec<u8>) -> Option<String> {
        if data.len() > MAX_TOTAL_BLOB_BYTES
            || self.total_bytes().saturating_add(data.len()) > MAX_TOTAL_BLOB_BYTES
        {
            return None;
        }
        let blob_id = format!("mqtt-blob-{}", Uuid::new_v4());
        self.blobs.insert(
            blob_id.clone(),
            ProviderBlob {
    ...
    /// invoke 结果按大小决定内联还是走 blob(消息页等大结果经 blob 分块读取)
    pub(crate) fn invoke_result(
        &mut self,
        resource_id: &str,
        value: Value,
        ...
        let Some(blob_id) = self.store(resource_id, data) else {
            return Err(boxed_error(
                error_codes::DATA_VALUE_OUT_OF_RANGE,
                "MQTT result exceeds the provider blob budget",
            ));
        };
    ...
    pub(crate) fn close(&mut self, params: BlobCloseParams) {
