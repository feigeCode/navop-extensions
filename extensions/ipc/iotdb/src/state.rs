...
impl CursorState {
    pub fn new(columns: Vec<ColumnSpec>, rows: Vec<Row>, max_rows: Option<u64>) -> Self {
        Self {
            columns,
            rows,
            next_row: 0,
...
impl ConnectionState {
    pub fn new(conn: IotDbSession) -> Self {
        Self {
            conn,
            cursors: HashMap::new(),
            streams: HashMap::new(),
    ...
    pub fn conn(&self) -> &IotDbSession {
        &self.conn
    }

    pub fn conn_mut(&mut self) -> &mut IotDbSession {
        &mut self.conn
    }

    pub fn open_cursor(&mut self, cursor: CursorState) -> CursorId {
        let cursor_id = format!("iotdb-cursor-{}", uuid::Uuid::new_v4());
...
#[allow(dead_code)]
pub fn _assert_conn_id(_: ConnId) {}
...
impl ImportState {
    pub fn new(table: String, columns: Vec<String>, options: ImportOptions) -> Self {
        Self {
            table,
            columns,
            options,
